...
async function createOverlappingTrips(
  userId1: string,
  userId2: string,
  destId: string,
) {
...
describe("connectionService", () => {
  test("cannot connect to yourself", async () => {
    ...
    await expect(
      connectionService.create(user.id, user.id),
  ...
  test("duplicate connection throws conflict", async () => {
    ...
    await expect(
      connectionService.create(sender.id, receiver.id),
    ).rejects.toMatchObject({
      statusCode: 409,
    });
  });
  ...
  test("only receiver can accept/reject", async () => {
    ...
    await expect(
      connectionService.updateStatus(
        sender.id,
        created.connection.id,
        "ACCEPTED",
      ),
