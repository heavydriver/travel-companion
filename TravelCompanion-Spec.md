...
| R-106 | Sign out                            | Clears access token from Zustand \+ React Native Async Storage. Clears refresh token cookie. Navigates to Onboarding.                                                                                   |
...
| R-402 | Store pack data in SQLite       | Places \+ phrases written to expo-sqlite tables. Mapbox SDK manages tile cache. Pack metadata (destinationId, packVersion, downloadedAt) stored in React Native Async Storage via offlineStore.                                       |
...
| R-705 | Favorite phrases    | Heart icon per card. Favorites persisted in React Native Async Storage (not synced to server). 'Favorites' tab filters to saved phrases. |
...
15. LanguageScreen \+ search \+ React Native Async Storage favorites.
...
| R-802 | Download resumes on interruption | Use FileSystem.createDownloadResumable. If download is interrupted, user can resume from where it stopped. Download state persisted in React Native Async Storage.                                                                                                                              |
...
| R-905 | Chat history persisted locally | Messages stored in React Native Async Storage (immediate, synchronous). Synced to server via POST /chat/messages in batches after each assistant response. Last 10 messages from React Native Async Storage included in every new LLM context window. |
| R-906 | Clear chat history             | Settings → 'Clear Chat History'. Confirmation modal. Clears React Native Async Storage chat cache \+ calls DELETE /chat/history on server.                                                                                                            |
...
* Bottom tab bar (5 tabs): Home · Explore · Map · Itinerary · Assistant
...
* React Native Async Storage for all mobile key-value storage.
...
| React Native Async Storage | React Native Async Storage. Used for all key-value storage on mobile                                                                          |
